...
SAIVerseは自宅のPCで動くアプリですが、**Tailscale**という無料のVPNサービスを使うと、外出先からスマホやタブレットで安全にアクセスできます。

さらに、ホーム画面にアプリとして追加すれば、ネイティブアプリのように快適に使えます（PWA機能）。
...
- [ ] SAIVerseがPCにインストール済み
- [ ] SAIVerseがPC上で正常に動作している（`http://localhost:3000` で開ける）
- [ ] PCとスマホがインターネットに接続できる
...
1. [Tailscaleのダウンロードページ](https://tailscale.com/download)を開く
2. Windowsなら「Windows」、Macなら「macOS」をクリックしてダウンロード
3. ダウンロードしたファイルを実行してインストール
4. インストールが終わると自動的にログイン画面が開くので、Googleアカウントなどでログイン

#### スマホへのインストール

1. App Store（iPhone/iPad）または Google Play ストア（Android）で「Tailscale」を検索
2. インストールしてアプリを開く
3. **PCと同じアカウント**でログイン（ここ重要！）

> **💡 ポイント**: PCとスマホで同じアカウントを使わないと、接続できません。
...
ブラウザで `http://localhost:3000` を開いて、SAIVerseの画面が表示されることを確認してください。

---

### 手順3：Tailscaleで公開する

SAIVerseをスマホからアクセスできるように公開します。

#### Windowsの場合

1. **スタートボタン**を右クリックして、「Windows ターミナル (管理者)」または「PowerShell (管理者)」を選択
2. 以下のコマンドを入力してEnter：
...
3. 「Serve started successfully」のようなメッセージが出ればOK
...
```
https://あなたのPC名.あなたのアカウント名.ts.net
```

このURLをメモしておいてください（スマホで入力します）。

> **💡 ヒント**: URLは `https://` で始まります。これがPWAインストールに必要な安全な接続です。

---

### 手順5：スマホでアクセスする

1. スマホで**Tailscaleアプリ**を開く
2. VPNが「Connected」になっていることを確認
3. スマホのブラウザ（SafariまたはChrome）を開く
4. 手順4で確認したURLを入力：
   ```
   https://あなたのPC名.あなたのアカウント名.ts.net
   ```
5. SAIVerseの画面が表示されれば成功！

---

### 手順6：ホーム画面に追加する（PWAインストール）

ブラウザで開くだけでも使えますが、ホーム画面に追加するとアプリのように快適に使えます。

#### iPhone / iPad（Safari）

1. 上記URLをSafariで開く
2. 下の「共有」ボタン（四角から矢印が出るアイコン）をタップ
...
これでホーム画面にSAIVerseのアイコンが追加され、タップするだけで起動できるようになります！
...
スマホからアクセスする必要がなくなったら、公開を停止できます：
...
- `start.bat` / `start.sh` で開いたウィンドウで `Ctrl+C` を押す
- またはウィンドウを閉じる
...
| 問題 | 確認すること |
|------|--------------|
| スマホでページが開かない | PCとスマホ両方でTailscaleアプリを開き、Connectedになっているか確認 |
| 「見つかりません」エラー | `tailscale serve status` でURLを再確認 |
| PCとスマホが繋がらない | 両方で**同じアカウント**でログインしているか確認 |
| PWAインストールが出ない | `https://...ts.net` のURLを使っているか確認（`http://` では不可） |
| 画像などが表示されない | PC上で `http://localhost:3000` が正常に動いているか確認 |

---

### 注意点

- **PCが起動している間だけ**アクセスできます。PCをスリープにしたり電源を切るとアクセスできなくなります。
- 複数のスマホ・タブレットで同時にアクセスすると、表示がおかしくなることがあります（改善予定）。
- Tailscaleは無料で使えますが、アカウント登録が必要です。

---

### もっと詳しく知りたい

- [Tailscale Runbook](./docs/getting-started/tailscale-runbook.md) - 詳しい手順書
- [Tailscale PWA 仕様書](./docs/features/tailscale-pwa.md) - 技術的な説明
...
- [Discord コミュニティ](https://discord.gg/qMcgEk83Ag) - 質問・雑談・バグ報告などお気軽にどうぞ
...
- **GitHub Sponsors**: [スポンサーページ](https://github.com/sponsors/maha0525)
