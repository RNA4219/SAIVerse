...
class TestLLMClients(unittest.TestCase):
    ...
    @patch('llm_clients.openai.OpenAI')
    def test_openai_client_generate_with_schema_skips_non_json_braces(self, mock_openai):
        mock_client_instance = MagicMock()
        mock_openai.return_value = mock_client_instance
        ...
        mock_resp.model_dump_json.return_value = '{}'
        mock_choice = MagicMock()
        mock_choice.message.content = '前置き {x}\n{"answer": "yes"}\n後置き'
        mock_choice.finish_reason = "stop"
        mock_resp.choices = [mock_choice]
        mock_client_instance.chat.completions.create.return_value = mock_resp
    ...
    @patch('llm_clients.openai.OpenAI')
    def test_openai_client_generate_with_schema_missing_required_key_raises(self, mock_openai):
        mock_client_instance = MagicMock()
        mock_openai.return_value = mock_client_instance
        ...
        mock_resp.model_dump_json.return_value = '{}'
        mock_choice = MagicMock()
        mock_choice.message.content = '{"oops": 1}'
        mock_choice.finish_reason = "stop"
        mock_resp.choices = [mock_choice]
        mock_client_instance.chat.completions.create.return_value = mock_resp
    ...
    @patch('llm_clients.openai.OpenAI')
    @patch('llm_clients.openai._validate_required_schema_keys', side_effect=NameError("missing"), create=True)
    def test_openai_client_generate_with_schema_does_not_depend_on_global_validator(self, _mock_validator, mock_openai):
        mock_client_instance = MagicMock()
        mock_openai.return_value = mock_client_instance
