...
def test_runtime_wrapper_rebinds_llm_module_helpers(monkeypatch) -> None:
    import sea.runtime_llm as runtime_llm

    monkeypatch.delattr(runtime_llm, "_format", raising=False)
    monkeypatch.delattr(runtime_llm, "_is_llm_streaming_enabled", raising=False)

    runtime_obj = runtime.SEARuntime(manager_ref=object())
    runtime_obj._lg_llm_node(SimpleNamespace(), SimpleNamespace(), "b1", SimpleNamespace(name="pb"))
